...
| =Shift+Insert=         | =paste_primary=   | Paste the primary selection |
| =Shift+Page_Up=        | =scroll_up=       | Scroll up one page          |
| =Shift+Page_Down=      | =scroll_down=     | Scroll down one page        |
| =Shift+Home=           | =scroll_top=      | Jump to top of scrollback   |
| =Shift+End=            | =scroll_bottom=   | Return to live output       |
...
| =scroll_up=        | Keyboard: one page up; mouse: configured step     |
| =scroll_down=      | Keyboard: one page down; mouse: configured step   |
| =scroll_top=       | Jump to the top of the scrollback buffer           |
| =scroll_bottom=    | Jump to the bottom (return to live output)         |
| =scroll_up_fast=   | Keyboard: three pages up                          |
| =scroll_down_fast= | Keyboard: three pages down                        |
...
=Hyper= and =Meta= are reserved enum values, not supported binding modifier names; the parser rejects them. Lock stripping assumes the conventional X11 assignments described below.
...
- *Shift+punctuation matching*: Keyboard events first match the translated symbol, then fall back to the active layout's unshifted symbol with the same modifiers when Shift is held. This lets =Ctrl+Shift+minus= and =Ctrl+Shift+0= work when the keyboard reports =underscore= and =parenright=. An explicit binding for the shifted symbol takes precedence.
- *Module matching*: Scroll actions, keyboard-selection activation, and external-pipe activation use the same matcher. Module text input keeps its translated symbols. Keyboard scroll actions move one page (or three pages for the fast variants); top/bottom actions jump to the corresponding history boundary. Mouse scroll increments are configured separately.
- *Backend symbols*: X11 and Wayland supply the active keymap's base symbol. LRG's Ctrl/Alt path translates Shift using raylib's US physical-key identifiers, including punctuation. Arbitrary layout translation in that LRG path is not provided by this mapping.
- *Regression coverage*: See [[file:keybinding-regressions.org][keybinding regression tests]] for the tested matrices and dispatch boundaries.
