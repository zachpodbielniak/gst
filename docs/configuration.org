...
| =editor=    | string  | =emacsclient= | executable and arguments | Waiting editor for command-output export |
...
gst_config_set_editor(config, "emacsclient");
...
| =gst_config_get_editor(config)= | =gst_config_set_editor(config, value)= |
...
=editor= is tokenized without shell expansion, and the private output filename is appended as one argument. Use a real executable, not a shell function, and do not pass =--no-wait=: the file is deleted when the editor process finishes. With =emacsclient=, finish using =C-x #=. Export requires active =shell_integration= and completed OSC 133 output; see [[file:history-workflows.org][history workflows]] and [[file:keybindings.org][keybindings]].
