...
:PROPERTIES:
:CUSTOM_ID: kitty-graphics-module
:END:
...
:CUSTOM_ID: overview
:END:
The kittygfx module implements the [[https://sw.kovidgoyal.net/kitty/graphics-protocol/][Kitty graphics protocol]], allowing programs to display images directly in the terminal. This enables image previews in file managers (e.g. yazi, ranger), inline plots in data analysis tools, and other graphical content within the terminal.
...
:PROPERTIES:
:CUSTOM_ID: continuation-chunk-id-resolution
:END:
...
:PROPERTIES:
:CUSTOM_ID: backend-compatibility
:END:
...
:PROPERTIES:
:CUSTOM_ID: dirty-state-and-repaints
:END:
...
- *Complete placeholder placement selection*: U+10EEEE virtual placements with =U=1= are supported in 0.4.7, but nonzero underline-color placement IDs are not.
- *Complete ordinary placement tracking*: Overwrite cleanup and region scrolling are supported in 0.4.7; ordinary placement resize/reflow and scrollback anchoring remain incomplete.
- *Shared memory object naming*: The =t=s= transfer path is gated by config but the actual shm_open logic is not implemented

See [[file:../graphics-parity.org][Graphics and LRG Integration]] for 0.4.7 placeholder behavior, overwrite semantics, backend support and the complete current limitations.
