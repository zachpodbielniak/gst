...
| =--recompile=      |          | Compile the C config file and exit; failure returns a nonzero status and never starts the terminal |
...
| =GST_MOUSE_DEBUG=     | If set, log every mouse event to stderr (press/release/motion, pixel->cell mapping, mouse-report vs. local-selection path). Diagnostic aid for click/drag/selection issues. |
