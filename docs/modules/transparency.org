...
With =--lrg=, the same settings fade the completed framebuffer, including text, backgrounds, borders and overlays. GST requests an alpha-capable window before creating the OpenGL context and submits premultiplied pixels to the compositor; it does not rely on GLFW's window-opacity API. This supports gowl, including the Xwayland path used by the vendored GLFW backend.
...
- *LRG*: Requires framebuffer alpha support and a compositing display server; no window-opacity protocol or gowl configuration change is needed.
